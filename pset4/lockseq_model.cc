...
constexpr std::array<std::string_view, 4>
    kLatencyPhaseNames = {"lock", "write", "clear", "unlock"};
...
void lockseq_model::record_latency(latency_phase phase,
                                   cot::steady_time_point start) {
    auto elapsed = std::chrono::duration_cast<std::chrono::nanoseconds>(
        cot::steady_now() - start);
    latency_samples_[static_cast<size_t>(phase)].push_back(elapsed);
}

void lockseq_model::print_latency_samples(std::ostream& out) const {
    for (size_t i = 0; i != latency_samples_.size(); ++i) {
        for (auto sample : latency_samples_[i]) {
            out << "latency_sample\t" << kLatencyPhaseNames[i] << '\t'
                << sample.count() << '\n';
        }
    }
}
...
cot::task<> lockseq_model::client(unsigned cid) {
    ...
    while (true) {
        ...
        auto lock_start = cot::steady_now();
        ...
        for (unsigned tries = 0; true; ++tries) {
            ...
            if (!resp) { // timeout; try a new leader every 3 retries
                ...
                record_latency(latency_phase::lock, lock_start);
                goto write_phase;
            }
    ...
    write_phase:
        ...
        while (n != nvalues) {
            ...
            serial += serial_step();
            auto write_start = cot::steady_now();
            for (unsigned tries = 0; true; ++tries) {
                ...
                record_latency(latency_phase::write, write_start);
                break;
            }
            ++n;
        ...
        while (n < max_sequence_length_) {
            std::string value_key = make_value_key(gid, n);
            serial += serial_step();
            auto clear_start = cot::steady_now();
            for (unsigned tries = 0; true; ++tries) {
                ...
                if (resp->errcode != pancy::errc::ok &&
                    resp->errcode != pancy::errc::not_found) {
                    throw resp->errcode;
                }
                // if this is our first try and there was nothing there,
                // then we must have reached the end of the previous client’s
                // values
                if (resp->errcode == pancy::errc::not_found && tries == 0) {
                    record_latency(latency_phase::clear, clear_start);
                    goto unlock_phase;
                }
    ...
    unlock_phase:
        ++clear_complete;
        serial += serial_step();
        auto unlock_start = cot::steady_now();
        for (unsigned tries = 0; true; ++tries) {
            co_await send_request<pancy::remove_request>(
            ...
            record_latency(latency_phase::unlock, unlock_start);
            break;
        }
